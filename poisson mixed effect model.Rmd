...
k.pm #overdispersion parameter -not a problem
```


Lower DIC so is a better fitting model. CHeck the RInla help pages for details on teh likelihoods but it is sparse
...
E(Dead_mites_i) = N_i*P_i (Number of trials * probability of event)
var(Dead_Mites_i) = N_i*P_i*(1-P_i)
...
Plot from the model to check th residuals
...
Remove the intercept becuase it makes it hard to see the other variables.
